...
- [x] **T1 propose_resume_edit tool** — section-scoped edits on ResumeContent
...
- [x] **T2 analyze_job_match tool** — read-only: loads a job listing (title, company,
...
- [x] **T3 Credly server-side cache** — badge fetch moves server-side with a 24h DB
...
- [x] **T4 Goal→resume→job thread** — system-prompt addendum: when a career goal is
...
- [x] **T5 Eval + gates** — extend sage-agent-eval with resume-edit and job-match

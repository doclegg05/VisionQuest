...
- [x] **T1 Schema + migration** — `SageMemory` (subject-scoped facts with `embedding
...
- [x] **T2 Validation gate** — `src/lib/sage/memory/schema.ts`: zod controlled vocab
...
- [x] **T3 Extraction** — `src/lib/sage/memory/extract.ts`: `extractMemories()` takes the
...
- [x] **T4 Retrieval** — `src/lib/sage/memory/retrieve.ts`: subject-scoped cosine search
...
- [x] **T5 Wiring** — `handlePostResponse()` gains fire-and-forget memory extraction
...
- [x] **T6 Consolidation cron** — `POST /api/internal/memory/consolidate` (Bearer
...
- [x] **T7 Teacher inspector** — API `GET/PATCH/DELETE /api/teacher/students/[id]/memories`
  (teacher/admin role-gated, AuditLog on correct/delete) + `MemoryInspectorPanel` in
  the student detail Progress tab.
- [x] **T8 Eval** — `scripts/sage-memory-eval.mjs`: replays 20 synthetic scripted
...
- [x] **T9 Gates + PR** — npm test / eslint / typecheck / build green; migration applied

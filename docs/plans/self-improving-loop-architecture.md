...
|---|---|---|---|
| `render.yaml` has **no `autoDeploy`** (defaults ON) | `git show fix/ci-direct-url:render.yaml \| grep -ci autoDeploy` | `0` | ✅ |
...
| promptfoo / stryker / coverage deps | `grep package.json` | **absent** → must be added (and to `dependencies`, not devDeps — Render skips devDeps) | ✅ (noted) |
| **NEW finding:** `ADMIN_DATABASE_URL` in `render.yaml` | read full `render.yaml` envVars | **NOT present** (only `DATABASE_URL`, `DIRECT_URL`) → **strengthens OD-1 to a hard blocker** | ⚠️ |
...
1. **The gates are the entire safety budget.** `render.yaml` has no `autoDeploy` field (defaults ON), `startCommand` runs `prisma migrate deploy` then boots, and `.github/workflows/ci.yml` has **no deploy step** — so today a merge to `main` deploys to 100% of prod **before/regardless of CI**. Ouroboros's first structural job (P-1) is to flip `autoDeploy:false` and make a CI-green deploy-hook the *only* deploy path. After that, CI (lint → audit:api-auth → typecheck → test → RLS-strict → build → smoke) + the new e2e + evals + canary burn-rate + post-deploy-verify literally replace the human reviewer — so their correctness, and their **unreachability by the fixer**, is non-negotiable.
...
|---|---|---|---|
| **Deploy gate** | `render.yaml` has **no `autoDeploy`** (defaults ON); `startCommand = prisma migrate deploy && node …server.js`; CI has **no deploy step** | `render.yaml`; `ci.yml` (single `verify` job) | **P-1 must set `autoDeploy:false` + deploy-hook-after-CI-green**, else "gates are the human" is bypassable |
...
| **Gate deps absent** | no promptfoo/stryker/coverage deps in `package.json` | `package.json` | Must add, into `dependencies` (Render skips devDeps) |
| **`ADMIN_DATABASE_URL` absent** | only `DATABASE_URL`,`DIRECT_URL` in `render.yaml` envVars | `render.yaml` | **OD-1 is a hard blocker:** `prismaAdmin` may fall back to `vq_app` → admin writes FAIL |
...
- **Wire the existing-but-unwired gates into CI:** `sage-rag-harness.mjs --strict` against a seeded pinned corpus (index-class); `test:e2e` (seeded fixture student) + `test:smoke:api` as a CI job; **promptfoo** stage with a **local-Ollama judge** + frozen Sage golden set (skill/AI-quality class); coverage (`node --experimental-test-coverage`). All new deps in `dependencies` (Render skips devDeps).
...
- Act runner sets Render `autoDeploy` OFF and triggers deploy via deploy-hook **only after CI is green**.
...
All `loop_*` tables: `visionquest` schema, **Pattern K admin-only RLS** (copy baseline `migration.sql:1871`), written **only** via `prismaAdmin` through the `recordLoopEvent` chokepoint, **redact-at-write**. `eval_case` is **CODEOWNERS-protected, NOT loop-writable.** **All loop migrations are expand/contract / forward-only-safe (§7.B).**
...
**`pg_cron` purge** (`net.http_post` pattern from baseline) enforces PII-free TTL on `loop_event`; `loop_ledger`/`eval_case` retained longer (PII-free by construction). **OPEN at P0 gate (OD-1, now hard blocker):** confirm `ADMIN_DATABASE_URL` (postgres role) is set in prod — **it is absent from `render.yaml` (§0)** — else `prismaAdmin` falls back to `vq_app` and admin-only writes **FAIL**.
...
|---|---|---|---|
| 0 | Render immutable + `/api/health` readiness | broken build/boot/migration (free first-line rollback) | render native |
...
**Auto-merge requires 1-12 green.** **Auto-deploy requires CI-green deploy-hook + killswitch unset + per-day cap not exceeded + spend-budget not exceeded (§11).** **Promotion to the next ring requires 13-14 green AND low rollback rate.**
...
The full global killswitch + single-writer deploy-lease are P3 artifacts, but P-1 flips `autoDeploy:false` and wires a deploy-hook, and P0 writes to prod DB via `prismaAdmin`. To eliminate the window where loop infra mutates prod with no off-switch:
...
**Files/tasks:** **re-run the §0 ground-truth table on the then-current deploy branch and paste outputs into the P-1 PR (blocking);** add `scripts/verify-ground-truth.mjs` CI check. Re-credential local claude CLI (rc=1 since 2026-05-08); restore Ollama/cloudflared/relay (`.state=UNHEALTHY`) via `warm-sage-model.ps1` + `start-sage-tunnel.bat`; run `codegraph init`; wire `logLlmCall` into provider stream paths (repairs token cap); add scrubbed `export const onRequestError = Sentry.captureRequestError` to `instrumentation.ts`; define server-side Sentry `beforeSend` scrub config (CODEOWNERS-protected + test); replace broken project-autopilot PostToolUse hook + add PreToolUse secret/PII redact gate + Stop parity check; **set `render.yaml autoDeploy:false`** + provision deploy-hook/`RENDER_*` (names only, `sync:false`); **confirm/provision `ADMIN_DATABASE_URL` (postgres role) — currently ABSENT from render.yaml (§0) [OD-1 BLOCKER]**; create `feature_flag` table early enough that the **interim killswitch (§7.A)** exists before any sensor write.
**Gates / success:** §0 table re-verified on deploy branch (any drift → STOP); all 4 local substrates LIVE (claude rc=0; Ollama HEALTHY; `LlmCallLog` rows populate `durationMs`/ttft with **no prompt/response text field**; Sentry receives a deliberately-thrown scrubbed route error); interim killswitch flips loop writes off; manual deploy fires **only** via deploy-hook after CI green; CI still green.
...
- **OD-1 (P-1/P0) [HARD BLOCKER]:** `ADMIN_DATABASE_URL` (postgres role) is **absent from `render.yaml` (§0)**. Provision it, or `prismaAdmin` falls back to `vq_app` and admin-only `loop_*` writes FAIL.

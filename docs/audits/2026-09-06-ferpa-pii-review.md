...
### 5.4 The identity vault: keep identifiers in a separate, harder-to-reach store and key everything else by an alias

Britt raised a pattern seen elsewhere: student PII collected on a separate form and held in a separate store (a Google Sheet, in the example), with a dashboard that overlays the data for visualisation without exposing the identifiers, and asked whether VisionQuest could separate intake from the app and refer to students by a secure alias. It can, the pattern has a name (the *data privacy vault* or split-knowledge architecture, `F-deidentification-architectures.md` §3), and this review recommends it as a step between Sprint 1 and Sprint 3.

**What VisionQuest already has.** Every student already carries a secure alias: `Student.id` is a random, unguessable cuid, and every transcript, goal, memory, mood entry and alert is keyed by it. The review's position (N12 in memo B) is that this id is defensibly not an identifier on its own: high-entropy, unenumerable, with the mapping to a person held only behind RLS. Row-level security already scopes who may read which rows.

**What it does not have.** The identity columns live in the same tables as the behavioural data, readable by the same `vq_app` role: `displayName`, `email`, `googleId` and `credlyUsername` on `Student` (`prisma/schema.prisma:15-21`); `firstName`, `lastName`, `referralEmail` and `birthDate` on `SpokesRecord` (`:356-367`); the phone number as `NotificationPreference.destination` (`:108`); `homeZip` on `StudentWorkProfile` (`:650`). A leak of the behavioural store is a leak of the identities. The cost of the split is measurable: **115 non-test source files read `displayName` and 48 read `email`**; each becomes a call to a narrow identity service instead of a column read.

**How the pattern maps onto VisionQuest.**

1. **Schema split.** Move the identity columns into a `StudentIdentity` table in its own Postgres schema, owned by a separate database role that `vq_app` cannot read (the same role-split discipline the repo already runs for `prismaAdmin`). The alias stays exactly as it is.
2. **One identity service.** A small module resolves alias to identity only for the surfaces that legitimately need a real person: login (by email or login id), the teacher's screen at render time (the "dashboard overlay"), SMS and email delivery, the employer packet, the DoHS export, and crisis alerts to instructors. Every resolution writes an audit row, which also satisfies the staff-read auditing rule (F16) in one place.
3. **AI lanes never receive vault columns.** With the identifiers in a schema the app role cannot read, the "no identifier in the prompt" claim becomes a structural guarantee enforced by a lint rule on the prompt builders, rather than a decorator that strips names after the fact. The pseudonymisation layer in §5.2 then only has to handle what the student types.
4. **Intake separated.** Whatever collects name, phone, address and date of birth (the existing WVDE/LACES intake, or a VisionQuest intake page writing only to the vault) issues the alias, and the behavioural side of VisionQuest receives nothing else. Sage greets the student by a preferred first name the student chooses, which can live outside the vault because the student controls it.

**Three honest limits.**

- **It is minimisation, not exemption.** 34 CFR 99.3(d) lists "a personal identifier, such as the student's ... student number" as PII, and the behavioural store remains an education record because the join key exists somewhere. The split shrinks what any one system exposes and what any one breach reveals; it does not take the transcript out of FERPA or W. Va. Code 9-9-20. The contract and consent in Parts 1 and 6 are still required.
- **Free text is untouched.** A student who types a cousin's phone number puts it in the behavioural store, alias or not.
- **The vault must be the most protected component, not the least.** A Google Sheet holding names and dates of birth is acceptable only inside a Workspace for Education tenant under the school-official terms (Part 4, §1c), never a personal account, and Sheets has weaker access control, audit and breach reporting than a Postgres role split. For VisionQuest the in-database split is cheaper, faster and stronger than an external sheet, and it keeps the number of processors at one.

**Effort and order.** Schema split plus identity service is a medium job, roughly a week, dominated by the 115 and 48 call sites. It belongs after Sprint 1 (so the refusal branch and audit events exist first) and before Sprint 3 (so the pseudonymisation layer is built against a codebase where the structured identifiers already cannot reach a prompt). The intake separation is a product and process decision that depends on how the LACES intake works today; it is D-K.
...
### Sprint 2b — the identity vault (engineering ~1 week, after Sprint 1, before Sprint 3)

21a. `StudentIdentity` table in its own schema and role; migration moving `displayName`, `email`, `googleId`, `credlyUsername`, `SpokesRecord.firstName/lastName/referralEmail/birthDate`, `NotificationPreference.destination` and `StudentWorkProfile.homeZip` behind it; a single audited identity service for login, teacher render, delivery, packets, export and crisis routing; a lint rule forbidding vault columns in any prompt builder (§5.4).
...
- **D-K. Identity vault and separated intake (§5.4):** (1) approve the in-database split (identity columns into their own schema and role, one audited identity service) as Sprint 2b; (2) decide whether intake moves out of VisionQuest entirely (the WVDE/LACES intake or a vault-only intake page issues the alias) or stays in-app writing only to the vault; (3) decide whether the student-chosen preferred first name that Sage uses lives outside the vault. Recommended: **yes to the split, in-app intake writing only to the vault for now, preferred name outside the vault.** Not recommended: a Google Sheet as the vault, unless it is inside a Workspace for Education tenant, and even then the Postgres split is stronger.

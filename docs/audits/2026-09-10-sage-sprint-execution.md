...
## Observation incident: bounded extraction transcripts

At 17:31 UTC, the production log review found two background errors from 17:02:26 UTC: `Discovery extraction failed` and `Memory extraction failed (non-fatal)`, both reporting Gemini's first-content role error. Health remained healthy at 17:31:55 UTC (695 ms database/schema check). These errors appeared after the immediate smoke log snapshot, so the first-hour gate has not passed. The observation heartbeat is paused while the fix is validated; observation windows must restart from the corrective release. No student conversation content was retrieved for debugging.

A bounded history can begin with Sage's model turn. Discovery and goal extraction appended an analysis request but left that leading model turn untouched. Student and staff memory extraction additionally ended with a model reply, which the Gemini chat adapter treated as the final user request. Four regression tests reproduce these defects. A shared transcript helper preserves every retained role/content pair, adds a user analysis request when the window begins with a model turn, and always ends with a genuine extraction request. Mood extraction already packages a labelled transcript as a user request; classroom extraction already has the corrected order. Neither needed another change.

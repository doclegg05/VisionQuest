...
- Any Sage behavior change must keep the gating CI evals green: red-team + chat harness in `.github/workflows/sage-evals.yml` (fixtures in `config/sage-*.json` assert verbatim prompt substrings and 988 handling). A model-backed eval that reads NOT RUN (the `GEMINI_EVAL_BUDGET_OK` guard, or a runtime `EvalBudgetExhaustedError` from a drained Gemini wallet — see `docs/runbooks/sage-eval-budget.md`) is not a pass and never substitutes for a real green run before merging

...
- **Where we left off**: `bug/english-method-intent-parity` committed locally, **not pushed and no PR opened**. It stacks on `bug/spanish-crisis-parity`, so the merge order is #136 → #139 → this branch; opening its PR against `main` before #139 lands would drag the Spanish commits in. Prior session's handoff still stands: draft PR #139 open (base: remediation/critical-high — do NOT merge before #136; retarget to main after it lands); repo CI triggers are main-only, so sage-evals were dispatched on-branch via workflow_dispatch (run 30281562637) — verify green. Remediation completion (29/29) is summarized in Current Status; its deploy checklist still stands in Open Items.

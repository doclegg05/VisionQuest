...
(2) **Workstream B** (local-AI on the iMac M4 32GB) on `local-ai/32gb-eval`: the candidate `gemma4:26b-a4b-it-qat` **passes S2** (clean 3-run mean 89.6%, worst 86.7% vs an 85% gate, 93.3% under CI-style majority voting) after two tool-description attractors and three uncallable eval fixtures were fixed. Full results, caveats and resume commands: `docs/plans/2026-07-27-local-ai-eval-results.md`.
...
- [ ] One real routing bug left: `file-cert-evidence` misses 3/3 — "put this with my certification records" drifts to lookup_cert_progress/find_certification. It IS callable, so this one is genuine (unlike submit-signed-dress)

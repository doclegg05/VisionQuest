---
type: form
title: Non-Discrimination Notice
description: The federal equal-opportunity notice that informs students of their right to participate in the program free from discrimination on the basis of race, color, national origin, sex, disability, age, or other protected characteristics.
resource: /api/forms/download?formId=non-discrimination&mode=view
tags:
  - non-discrimination
  - equal-opportunity
  - civil-rights
  - compliance
  - onboarding
timestamp: '2026-06-30'
vq_id: non-discrimination
vq_audience: STUDENT
vq_category: onboarding
vq_storage_key: >-
  teachers/guides/Hanbook Appendix/Section 4/WVAdultEd Sample Non-Discrimination
  Notice.pdf
vq_status: draft
---
## When to use
During orientation, when a student must be given the required federal notice of their equal-opportunity rights in the program.

## When NOT to use
- NOT for general program rules or student expectations — those are in [Rights and Responsibilities](./rights-responsibilities.md).
- NOT a release-of-information form — for information sharing see [Authorization for Release of Information](./auth-release.md) or [DoHS Release of Information](./dohs-release.md).

## Related
Provided at orientation alongside [Rights and Responsibilities](./rights-responsibilities.md) as part of the required federal compliance acknowledgments.

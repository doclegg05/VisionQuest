...
export default function NavBar({ studentName, role, navPhase, orientationComplete }: NavBarProps) {
  ...
  const primaryItems =
    role === "student"
      ...
      : role === "admin"
        // Admin has their own Sage (/admin/chat) and Library (/admin/library).
        // Drop the teacher-facing duplicates from STAFF_ITEMS so the sidebar
        // doesn't show "Sage" or "Library" twice.
        ? [
            ...ADMIN_ITEMS,
            ...COORDINATOR_ITEMS,
            ...STAFF_ITEMS.filter(
              (item) => item.href !== "/teacher/chat" && item.href !== "/teacher/library",
            ),
          ]
        : role === "coordinator"
          ? COORDINATOR_ITEMS
        ...
        <>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
              ...
              {/* Settings in More sheet for every role that has one */}
              {settingsHref && (
                <Link
                  href={settingsHref}
                  onClick={() => setMoreOpen(false)}

...
//
// Rate-limit, audit, and the downstream ownership read are mocked so the
// mode-check-passes cases cannot touch a database.
mock.module("@/lib/db", {
  namedExports: {
    prisma: { certRequirement: { findFirst: async () => null } },
    prismaAdmin: {},
  },
});
...
describe("executeAgentTool — independent mode re-check", () => {
  ...
  it("allows a mutate_consequential tool call when mode is full (downstream ownership still applies)", async () => {
    setMode("full");
    const record = await executeAgentTool({
      session: studentSession,
      conversationId: "conv-1",
      toolName: "mark_certification_complete",
      args: { requirementId: "req-1" },
      // Mocked missing requirement rejects after the mode check passes.
    });
    assert.equal(record.result.status, "error");
    assert.match(record.result.summary, /wasn't found on your account/);
  });
});

...
// ---------------------------------------------------------------------------
// Backup-code regeneration route — request-level tests
//
...
let backupCodesRoute: Awaited<typeof import("../route")>;

before(async () => {
  backupCodesRoute = await import("../route");
});

function regenerateRequest(token: string) {
  return mockRequest("/api/auth/mfa/backup-codes", { method: "POST", body: { token } });
}
...
describe("POST /api/auth/mfa/backup-codes", () => {
  ...
  it("returns 429 when the per-account limit is exhausted", async () => {
    ...
    assert.deepEqual(mockRateLimit.mock.calls[0]?.arguments, [`mfa-backup:${teacher.id}`, 3, 5 * 60 * 1000]);
    assert.equal(mockFindUnique.mock.callCount(), 0);
  });
